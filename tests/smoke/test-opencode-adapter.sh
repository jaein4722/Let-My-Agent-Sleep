...
FAIL_OUTPUT=$(cd "$ROOT" && LMAS_RUNS_DIR="$RUNS_DIR" LMAS_OPENCODE_SESSION_ID="session-500" LMAS_OPENCODE_SERVER_URL="http://127.0.0.1:$FAIL_PORT" ./packages/let-my-agent-sleep/bin/lmas.sh start --adapter opencode -- ./examples/fake_train.sh success)
FAIL_RUN_ID=$(printf '%s\n' "$FAIL_OUTPUT" | awk '/^run_id:/ { print $2 }')
FAIL_RUN_DIR="$RUNS_DIR/$FAIL_RUN_ID"

for _ in $(seq 1 100); do
  [ -f "$FAIL_RUN_DIR/adapter.log" ] && break
  sleep 0.1
done
