...
- **A live owner is optional, not something LMAS installs.** OpenCode hands completion to an existing `opencode serve`. Codex first tries an existing built-in `codex app-server --listen unix://` owner, then another built-in live owner, then `codex exec resume`. Claude registers `lmas await` through its own native background-task harness and uses `claude --resume` only when the waiter or its exact owning Claude process is definitely absent or dead. LMAS never installs or starts an agent server, daemon, executable wrapper, or PATH change. Every adapter preserves `resume_prompt.txt` when no live owner is available. If the machine reboots, the training job dies with it — `LOST` is the correct terminal representation of that situation, not a recovery gap to fix.
...
- **Claude Code arbitrary post-hoc injection:** still requires an upstream session API. LMAS intentionally uses the narrower native background-task completion callback instead: pre-register one lightweight waiter while tmux owns the durable job, then fall back only on definite pre-delivery failure.
