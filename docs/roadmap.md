...
- **A live owner is optional, not something LMAS installs.** OpenCode hands completion to an existing `opencode serve`. Codex first tries an existing built-in `codex app-server --listen unix://` owner (including a TUI attached with `codex --remote unix://`), then Desktop, then `codex exec resume`; Claude uses `claude --resume`. LMAS never installs or starts an agent server. Every adapter preserves `resume_prompt.txt` when no live owner is available. If the machine reboots, the training job dies with it — `LOST` is the correct terminal representation of that situation, not a recovery gap to fix.
